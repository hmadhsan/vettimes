...
export default {
  ...
  methods: {
    ...
    getUserAlerts: function() {
      this.http.post('useralerts', {'action': 'getAlerts'}).then( r => {
        if ( this.$error(r.data) ) {
          this.alerts = r.data.array;
        }
      }).catch((e) => {
    ...
    getUserCourses: function() {
      this.http.get("course/users?_path=" + this.$route.path).then( r => {
        if(r.data.status) {
          this.courses = r.data.array;
          this.isCourses = true;
        } else {
          this.courses = [];
    ...
    deleteCourse: function(course_id) {
      if(store.state.auth) {
        this.http.post('usercourses', {  action: 'deleteCourse' ,course_id: course_id }).then( r => {
          store.commit({
            type: 'changeStars',
            stars: r.data.data
          });
